...
- (void) playDef:(UIButton *)sender
{
    if (search.isFirstResponder) {
        ...
        if (!search.isHidden) {
            ...
            MusicWord *nowWord = [MusicWord findById:sender.tag userId:nowUserId];
            wordPlayerTwo =[[AVPlayer alloc]initWithURL:[NSURL URLWithString:nowWord.audio]];
            [wordPlayerTwo play];
            [nowWord release];
            UIFont *Courier = [UIFont fontWithName:@"Courier" size:12];
            if ([sender.titleLabel.text isEqualToString:kWordOne]) {
                ...
                [sender setTitle:[MusicWord findById:sender.tag userId:nowUserId].def forState:UIControlStateNormal];
                [MusicWord addCheck:sender.tag userId:nowUserId];    
            }
        }
    }
}
...
- (void) playWord:(UIButton *)sender
{
    ...
    MusicWord *nowWord = [wordsArray objectAtIndex:sender.tag];
    wordPlayerTwo =[[AVPlayer alloc]initWithURL:[NSURL URLWithString:nowWord.audio]];
    [wordPlayerTwo play];
//    NSLog(@"play word");
}
...
//            NSLog(@"添加生词到:%d",nowUserId);
            words = [MusicWord findWords:nowUserId];
...
- (void)doSearch
{
    ...
    [search setText:@""];
    if (isiPhone) {
        [search setFrame:CGRectMake(0, 0, 320, 65)];
        [wordsTableView setFrame:CGRectMake(0, 65, 320, 351+44)];
    }else {
        [search setFrame:CGRectMake(0, 0, 768, 65)];
        [wordsTableView setFrame:CGRectMake(0, 65, 768, 351+44+544)];
    }
...
- (void) viewWillAppear:(BOOL)animated {
    ...
    if (isiPhone) {
        [contentBg setFrame:CGRectMake(9, 0, 302, self.view.frame.size.height-7)];
               [wordsTableView setFrame:CGRectMake(10, 0, 300, self.view.frame.size.height-17)];
        
        [search setFrame:CGRectMake(0, 0, 320, 44)];
    }else {
        [wordsTableView setFrame:CGRectMake(22, 0, 724, 865)];
        [search setFrame:CGRectMake(0, 0, 768, 44)];
    }
    self.navigationController.navigationBarHidden = NO;
	[search setHidden:YES];
    NSArray *delWords = [[NSArray alloc] init];
    delWords = [MusicWord findDelWords:nowUserId];
    for (MusicWord *fav in delWords) {
//        NSLog(@"删除:%@",fav.key);
...
//    [segButton release];
	...
	[imgBtn setTitle:kWordNine forState:UIControlStateNormal];
	imgBtn.titleLabel.font=[UIFont systemFontOfSize:13];
    [imgBtn addTarget:self action:@selector(doTong) forControlEvents:UIControlEventTouchUpInside];
    imgBtn.frame = CGRectMake(0, 0, 70, 34);
    UIBarButtonItem * tongbutton = [[UIBarButtonItem alloc] initWithCustomView:imgBtn];
    self.navigationItem.leftBarButtonItem = tongbutton;
...
- (void)dealloc {
    ...
    [self.myWord release];
    [self.defSLabel release];
    [self.search release];
    [self.explainView release];
//    [self.HUD release];
    [self.wordFrame release];
...
//    UIFont *CourierTwo = [UIFont fontWithName:@"Arial" size:18];
    UIFont *CourierThree = [UIFont fontWithName:@"Arial" size:12];
    MusicWord *word = [wordsArray objectAtIndex:indexPath.row];
    [word addRemember];
...
//    [cell.audioButton setTitle:@"播" forState:UIControlStateNormal];
    ...
    [cell.defButton.titleLabel setFont:CourierThree];
...
{	
    MusicWord *word = [wordsArray objectAtIndex:indexPath.row];
	[MusicWord deleteWord:word.key userId:nowUserId];
    ...
    if (search.isFirstResponder) {
        ...
        if (!search.isHidden) {
            ...
            if ([mylabel.text isEqualToString:kWordOne]) {
                [mylabel setText:[MusicWord findById:mylabel.tag userId:nowUserId].def];
                [MusicWord addCheck:mylabel.tag userId:nowUserId];    
            }
        }   
    }
}
...
{
//    NSLog(@"!!!!!!!_____________!!!!!!!!!!!");  
    MusicWord *nowWord = [MusicWord findById:wordId userId:nowUserId];
    if ([nowWord.def isEqualToString:@""]){
//        NSLog(@"下载");       
//        NSLog(@"Queue 预备: %d",wordId);
        NSOperationQueue *myQueue = [WordViewController sharedQueue];
...
//                NSLog(@"pron:%@", pron);
//                NSLog(@"def:%@", def);
                [MusicWord updateBykey:key audio:audio pron:pron def:def userId:nowUserId];
            }
...
//                NSLog(@"word:%@", word);
                if (result==1) {
                    if (request.tag < 0) {
//                        NSLog(@"删除:%@",word);
                        [MusicWord deleteByKey:word userId:nowUserId];
                    }
                    else{
                        [MusicWord updateFlgByKey:request.username userId:nowUserId];
                    }
                }
            }
        }
...
//                        NSLog(@"4");
                        for (DDXMLElement *objOne in itemsOne) {
                            NSString *orig = [[[[objOne elementForName:@"orig"] stringValue]stringByReplacingOccurrencesOfString:@"<em>" withString:@""] stringByReplacingOccurrencesOfString:@"</em>" withString:@""];
                            [eng addObject:orig];
                            NSString *trans = [[objOne elementForName:@"trans"] stringValue];
                            [chi addObject:trans];
//                            NSLog(@"orig:%@", orig);
//                            NSLog(@"trans:%@", trans);
                        }
                    ...
                    UILabel *defLabel = [[UILabel alloc]initWithFrame:CGRectMake(0, 20, self.explainView.frame.size.width, lines*20)];
                    ...
                    UILabel *dictLabel = [[UILabel alloc]initWithFrame:CGRectMake(0, 20+lines*20, self.explainView.frame.size.width, totalNum*20)];
...
//                NSLog(@"audio:%@", audio);
//                NSLog(@"pron:%@", pron);
//                NSLog(@"def:%@ %d", def,[MusicWord findLastId]+1);
                MusicWord *nowWord = [[MusicWord alloc] initWithMusicWord:([MusicWord findLastId]+1) key:key audio:audio pron:pron def:def date:nil checks:0 remember:0 userId:nowUserId flag:0];
//                NSLog(@"初始化");
...
//            NSLog(@"%d,同步圆满完成。",request.tag);
            [MusicWord deleteSynchro:nowUserId];
            NSArray *words = [[NSArray alloc] init];
            words = [MusicWord findWords:nowUserId];
            [wordsArray removeAllObjects];
            for (id fav in words) {
